<template>
  <a-form :form="form" @submit.prevent="handleSubmit">
    <a-form-item label="二维码替换" :label-col="labelCol" :wrapper-col="wrapperCol">
      <a-upload
...
export default {
    ...
    handleSubmit () {
      this.form.validateFields((err, value) => {
        this.$post('backend/active/addActiveConf', {
          qrcode: this.fileList.length > 0 ? this.fileList[0].url : ''
        }).then(res => {
          if (res) {
  ...
  watch: {
    formData (val) {
      this.setForm(val)
      this.qiniuToken = localStorage.getItem('qiniuToken')
    }
  }
}
</script>
